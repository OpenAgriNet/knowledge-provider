...
class TestPublishedValidityWindow:
    """The lifetime the prod approver set travels on the catalog it publishes."""

    @pytest.mark.unit
    def test_envelope_carries_the_approvers_window(self, monkeypatch):
        ...
        with patch("pipeline.discovery_publish_service.httpx.Client", return_value=client):
            result = service.publish(
                transaction_id="txn-validity",
                document_kind="advisory",
                validity=ValidityWindow(start_date="2026-09-16", end_date="2026-12-31"),
            )

        catalog = client.post.call_args.kwargs["json"]["message"]["catalogs"][0]
        assert catalog["validity"] == {
            "startDate": "2026-09-16T00:00:00Z",
            "endDate": "2026-12-31T23:59:59Z",
        }
        # The recorded envelope is what an operator inspects after the fact.
        assert result["envelope"]["message"]["catalogs"][0]["validity"] == catalog["validity"]

    @pytest.mark.unit
    def test_omits_validity_when_the_document_has_none(self, monkeypatch):
        # Pre-existing behaviour for documents promoted before windows existed.
        service = _service(monkeypatch)
        client = _client_returning(
            _on_publish_response("cat-oan-knowledge-provider-advisories", "ACCEPTED")
        )
        ...
        with patch("pipeline.discovery_publish_service.httpx.Client", return_value=client):
            service.publish(transaction_id="txn-none", document_kind="advisory")

        catalog = client.post.call_args.kwargs["json"]["message"]["catalogs"][0]
        assert "validity" not in catalog

    @pytest.mark.unit
    def test_a_window_does_not_make_an_unmapped_kind_publishable(self, monkeypatch):
        from pipeline.network_validity import ValidityWindow

        service = _service(monkeypatch)
        client = _client_returning(MagicMock())

...
```bash
# Run the full stack (api, worker, temporal, minio, qdrant, ui)
...
- `document_validity.py` — pure module owning Document Validity: the period a document's chunks answer searches in, the upload-day/+1-year default, and the clock injection point. Shared by `db.py` (stamping on upload), `scheme_catalog.py` (validating a reviewer's edit), `activities.py` (stamping chunk payloads) and `vector_store/qdrant_store.py` (filtering at search time)
